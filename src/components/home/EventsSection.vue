...
<template>
  ...
  >
    ...
    <div class="flex flex-col gap-[64px] items-center w-full relative z-[1]">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-5 w-full max-w-none px-[128px] max-lg:px-8 max-sm:px-4">
        <article
          v-for="(item, idx) in events"
          :key="idx"
          class="relative flex flex-col justify-center w-full h-[420px] md:h-[460px] overflow-hidden rounded-[4px] border-[0.711px] border-[#f0c24f] cursor-pointer group transition-shadow duration-300 hover:shadow-[0_0_40px_rgba(240,194,79,0.25)]"
        ...
        >
          ...
          <!-- 内容 -->
          <div class="relative z-10 flex flex-col gap-[8px] max-sm:gap-[6px] px-6 lg:px-8 pt-2 lg:pt-3 pb-6 lg:pb-8 w-full">
            <!-- 状态标签 -->
