...
import { orbDefaults } from '@/config'

// GLSL shaders — 从 reactbits Orb 组件移植
const vert = /* glsl */ `
...
export function useOrbEffect(containerRef, options = {}) {
  const opts = { ...orbDefaults, ...options }
  let rafId = null
  let renderer = null
  let gl = null
  ...
  onMounted(() => {
    ...
    function update(t) {
      ...
      const effHover = opts.forceHoverState ? 1 : targetHover
      program.uniforms.hover.value += (effHover - program.uniforms.hover.value) * 0.1
      if (opts.rotateOnHover && effHover > 0.5) currentRot += dt * rotationSpeed
